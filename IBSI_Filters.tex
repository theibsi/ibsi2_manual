...
This reference manual draws on and extends the first IBSI reference manual~\cite{ZLV2017} by defining and standardising image biomarkers based on image filters, \textit{e.g.}, wavelets, Laws kernels and Laplacians of Gaussian (LoG).
...
The first section of this chapter introduces notations and conventions (Section~\ref{sec:notations}).
The second section provides a brief overview of the image filtering process for image biomarker extraction (Section~\ref{sec:overview}).
...
Practical considerations, recommendations and data processing workflows of filtering for radiomics are provided in Chapter~\ref{sec:practicalConsiderations}.
...
The discrete Fourier transform\footnote{In this discrete setting, the normalised Nyquist frequency is denoted by $\pi$.} of a summable $f[\boldsymbol{k}]$ is noted $\hat{f}[\boldsymbol{\nu}]\in\mathbb{C}$, with $\boldsymbol{\nu}=(\nu_1,\dots,\nu_D) \in \mathbb{Z}^D$ the discrete frequency coordinate vector, and is defined as
...
\end{equation}
%
where the standard deviation of the Gaussian $\sigma$ controls the scale of the operator. Note that \(\sigma\) is implied to be in voxel units.
...
An easy way to implement several consecutive iterations of such a non-separable wavelet transform is to start from the initial definition of the band-pass filter (the one with highest frequency band, e.g. Eq.~\eqref{eq:simoncelliWavelet}), and constructing the filter with half $\nu_B$ at every consecutive step. The response map can then be computed by taking the Hadamard product of the bandpass filter and the image in Fourier domain, and then performing an inverse Fourier transform.

As described above, the support of the wavelet filter must be square/cubic as well as equivalent to the image to perform the isotropic convolution operation in the Fourier domain via a Hadamard product. When a collection of images is used for instance in the context of batch feature extraction over a cohort of patients, we recommend to fix image size to a square/cubic bounding box that can contain all ROIs in the dataset , after image isotropic image resampling (see Chapter~\ref{sec:practicalConsiderations}). This will ensure that wavelet scales are comparable accross patients as well as truly isotropic and rotation invariant image analysis.
...
However, these non-separable circularly symmetric wavelets can be advantageously combined with directional analysis methods such as directional derivatives or spherical harmonics.

An elegant approach to obtain features measuring directional transitions between pixel values (\textit{e.g.} gradient- or Hessian-like) is to compute them in the Fourier domain instead of using pixel differences in Gaussian-smoothed images~\cite{DeF2017}.
...
Computing $l^{\text{th}}$-order derivatives has an intuitive interpretation (\emph{e.g.}, gradient for $l=1$, curvature for $l=2$), which makes them attractive for understanding the meaning of the texture measures in a particular medical or biological application context.
However, a pure image derivative filter as computed in Eq.~(\ref{eq:FourierDerivative}) is high-pass (because multiplied by $\nu_i$) and accentuates high frequencies along $k_i$.
Therefore, it is desirable to implement image derivatives as all-pass filters, which is what the 1st-order Riesz transform yields $\boldsymbol{\mathcal{R}}\{f\}[\boldsymbol{k}]$ as~\cite{UnV2010}
%
...
\end{table}
%
\chapter{Filtering for Radiomics in Practice}\label{sec:practicalConsiderations}
%

Filtering is part of image processing for radiomics. When applied to an image, displacements of one voxel may not correspond to a displacement of the same physical distance depending on the direction. 
...
\endlastfoot
    ...
    \begin{itemize}[nosep,after=\strut,leftmargin=*]
        \item zero padding
        \item 3$D$ filter, E5L5S5 energy map, distance $\delta = 7$ voxels
        \item 3$D$ rotation invariance, \texttt{max} pooling
    \end{itemize}
    \end{minipage} \\ \cline{4-4}    
    3.b.1 & & checkerboard &
    ...
    \begin{itemize}[nosep,after=\strut,leftmargin=*]
        \item mirror padding
        \item 3$D$ filter, E3W5R5 energy map, distance $\delta = 7$ voxels
        \item 3$D$ rotation invariance, \texttt{max} pooling
    \end{itemize}
    \end{minipage} \\ \cline{4-4}    
    3.c.1 & & checkerboard &
    ...
    \begin{itemize}[nosep,after=\strut,leftmargin=*]
        \item mirror padding
        \item 2$D$ filter, L5S5 energy map, distance $\delta = 7$ voxels
        \item 2$D$ rotation invariance, \texttt{max} pooling
    \end{itemize}
    \end{minipage} \\
    \midrule
...
\end{tabular}
\normalsize
\caption{Image processing configurations for obtaining reference values for intensity-based statistical features and Intensity Histogram (IH) features after image filtering in the lung cancer CT image. The different processing steps are described in the IBSI reference manual~\cite{Zwanenburg2020-jt}. Note that the image should not be cropped. ROI: Region Of Interest; HU: Hounsfield Unit; FBN: Fixed Bin Number.
...
\begin{table}[h!]
    ...
    \begin{minipage}[t]{\linewidth}
    \begin{itemize}[nosep,after=\strut,leftmargin=*]
        \item 3$D$ filter, L5E5E5 energy map, distance $\delta = 7$ voxels
        \item 3$D$ rotation invariance, \texttt{max} pooling
    \end{itemize}
    \end{minipage} \\ \hline
    5.A | 5.B & Gabor & \(\times\) & \(\times\) & 
    \begin{minipage}[t]{\linewidth}
    \begin{itemize}[nosep,after=\strut,leftmargin=*]
        \item 2$D$ modulus response map
...
\section{Troubleshooting and Recommendations}\label{sec:troubleshootRecommend}
This sections contains several directions to troubleshoot your filtering implementations, as well as recommendations to get most of a considered filter family in a radiomics application.

\subsection{Rotation invariance and separable wavelets}
A first recommendation concerns the use of separable wavelets for rotation invariant image analysis. We extend here the discussion initiated in Section~\ref{sec:waveletConsiderations}. 
It was highlighted that separable wavelets were initially designed for image compression and coding efficiency and their use for image analysis (\emph{e.g.} radiomics) deviates from their initial purpose. In particular, they yield many response maps that are biased towards image axes and lack rotational invariance.
Nevertheless, approximate rotationally invariant image analysis using separable wavelets can be achieved using (voxelwise) orientation pooling over equivariant (\emph{e.g.} right angle) representations as suggested in Section~\ref{sec:combiningDSandRotInv} for the first level of the decomposition.
However, applying orientation pooling after every convolution operation of a multi-level separable wavelet decomposition would result in an overly complicated algorithm, yielding only approximate rotational invariance where the granularity of the approximation depends on the number of group elements (\emph{e.g.} right angles only or using more angular samples) in the used equivariant representation.
...
We recommend to use non-separable isotropic wavelets to achieve a true and multi-level rotation-invariant image analysis by design (see Section~\ref{sec:waveletConsiderations}).
